...
```{r}
# Tidak perlu dijalanin karena library readr udah include di tidyverse
# library(readr)
```
...
```{r}
data_orang = read.csv("data_orang.csv")
```

Bisa ambil data dari internet

```{r}
data_mobil = read_csv("https://raw.githubusercontent.com/r-lib/vroom/master/inst/extdata/mtcars.csv")
```

### Menggunakan read_excel dari library readxl untuk format .xlsx / .xls
